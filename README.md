...
Replaces the traditional one-module-panel workflow with a procedural,
...
multi-step workflows (load → register → resample → segment → render),
that loop becomes the bottleneck.

This extension keeps Slicer's MRML scene and all its modules — but
exposes them through a graph editor at the bottom of the main window.
Each node wraps a Slicer module; edges carry typed MRML data (volumes,
...
without a backing module. Pressing `1`–`0` routes a node's output to
one of 10 viewer slots — like Nuke.
...
| **Viewer routing** | 10 slots (`1`–`9`, `0`); per-slot badge color; visibility scoping (the volume you're viewing keeps its VR; others hide) |
...
includes 4D sequence support, `.mrb`-native graph persistence,
auto-recompute when upstream MRML changes (currently you press `1` to
re-evaluate), and a generic SimpleFilter node to expose all 18 of
Slicer's CLI Filtering modules through one configurable node.
...
5. Restart Slicer.
6. The "Node Editor" module will be available under the **Utilities**
...
6. Double-click the Volume Rendering node. The left panel switches to
   Slicer's native Volume Rendering module — pre-configured for
   MRHead.
7. Press `1` with the Volume Rendering node selected. Slicer switches
   ...
   shift slider — you're back in normal Slicer.
...
- **`SlicerNodeEditor/`** — the Slicer scripted module. Entry point
  (`SlicerNodeEditor.py`) creates the canvas dock and the properties
  panel.
- **`SlicerNodeEditor/NodeGraph/`** — the graph engine. Pure
...
- **`SlicerNodeEditor/Nodes/`** — the wrappers. Each file groups
...
- **`docs/design/procedural_workflow_analysis.md`** — the long-form
...
For "I want to add a node for module X" — extend `LinkedModuleNode`,
set `LINKED_MODULE = "ModuleName"`, declare `INPUT_PORTS` /
`OUTPUT_PORTS`, and supply an `INPUT_SETTERS` dict mapping each input
port to the module widget's setter method
(e.g. `'volume_in': 'setMRMLVolumeNode'`). The base class handles
configuring the module widget when the node is selected.
...
- Qt binding: **PythonQt**. PySide2 / PySide6 also supported via the
